...
| `watch` | Reactive hand-off: poll a read verb (`uses:`) every `every:` and run a hand-off action when an `on:` rule's `if:` holds — `handoff.bail` (reason gone → tear down), `handoff.rerun_step` (re-run this step), or `handoff.run_workflow` (run a named workflow — re-review). Operator-owned. See [[Hand-offs]]. |
