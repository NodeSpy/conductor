...
func (st *packInstantiation) applyTriggerOverlay(ns string, inst PackInstance, trs []TriggerSpec, armName func(int) string) error {
	...
	for i := range trs {
		name := armName(i)
		// An instance's identity is INTERNAL — a content hash keying its
		// dedup/session/outcome state, never an address. There is no syntax
		// for singling one out, and nothing to invent one for: instances are
		// array entries in the consumer's own `triggers:` block, edited where
		// they sit. So the overlay is keyed on the address the author WROTE,
		// and applies to every instance of it — they are one trigger fanned
		// out, and a filter meant for it is meant for all of them.
