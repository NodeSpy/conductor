...
// kvInstance builds a registry from an empty config and returns the
...
func TestKVVerbSurface(t *testing.T) {
	...
	out := call("get", map[string]any{"key": "missing"})
	if out["found"] != false || out["value"] != nil {
		t.Fatalf("absent get: %v", out)
	}
	out = call("get", map[string]any{"key": "missing", "default": "fb"})
	if out["value"] != "fb" || out["found"] != false {
		t.Fatalf("default: %v", out)
	}

	// set + get round-trip with namespace + structured value.
	call("set", map[string]any{"namespace": "pd", "key": "last", "value": map[string]any{"id": "i-1"}})
	out = call("get", map[string]any{"namespace": "pd", "key": "last"})
	if out["found"] != true || out["value"].(map[string]any)["id"] != "i-1" {
		t.Fatalf("round-trip: %v", out)
	}

	// setnx created vs existing.
	out = call("setnx", map[string]any{"key": "lock", "value": "a"})
	if out["created"] != true || out["value"] != "a" {
		t.Fatalf("setnx create: %v", out)
	}
	out = call("setnx", map[string]any{"key": "lock", "value": "b"})
	if out["created"] != false || out["value"] != "a" {
		t.Fatalf("setnx existing: %v", out)
	}

	// merge upsert + type check.
	out = call("merge", map[string]any{"key": "obj", "value": map[string]any{"a": 1}})
	if out["value"].(map[string]any)["a"] != 1 {
		t.Fatalf("merge: %v", out)
	}
	if _, err := in.Invoke(ctx, "merge", map[string]any{"key": "obj", "value": "nope"}); err == nil || !strings.Contains(err.Error(), "must be an object") {
		t.Fatalf("merge non-object option: %v", err)
	}

	// incr default by=1 and explicit by.
	if out := call("incr", map[string]any{"key": "n"}); out["value"] != int64(1) {
		t.Fatalf("incr: %v", out)
	}
	if out := call("incr", map[string]any{"key": "n", "by": 4}); out["value"] != int64(5) {
		t.Fatalf("incr by: %v", out)
	}

	// append item vs items, unique; len output.
	out = call("append", map[string]any{"key": "l", "item": "x"})
	if out["len"] != 1 {
		t.Fatalf("append item: %v", out)
	}
	out = call("append", map[string]any{"key": "l", "items": []any{"y", "x"}, "unique": true})
	if out["len"] != 2 {
		t.Fatalf("append unique: %v", out)
	}
	if _, err := in.Invoke(ctx, "append", map[string]any{"key": "l"}); err == nil || !strings.Contains(err.Error(), "item") {
		t.Fatalf("append needs item|items: %v", err)
	}

	// contains / remove / list accessors.
	if out := call("contains", map[string]any{"key": "l", "item": "y"}); out["contains"] != true {
		t.Fatalf("contains: %v", out)
	}
	out = call("remove", map[string]any{"key": "l", "item": "x"})
	if out["len"] != 1 || !reflect.DeepEqual(out["value"], []any{"y"}) {
		t.Fatalf("remove: %v", out)
	}
	call("append", map[string]any{"key": "l", "items": []any{"z", "w"}})
	if out := call("first", map[string]any{"key": "l"}); out["value"] != "y" || out["found"] != true {
		t.Fatalf("first: %v", out)
	}
	if out := call("last", map[string]any{"key": "l"}); out["value"] != "w" {
		t.Fatalf("last: %v", out)
	}
	if out := call("index", map[string]any{"key": "l", "index": -2}); out["value"] != "z" {
		t.Fatalf("index: %v", out)
	}
	if out := call("index", map[string]any{"key": "l", "index": 9}); out["found"] != false || out["value"] != nil {
		t.Fatalf("index out of range: %v", out)
	}
	out = call("slice", map[string]any{"key": "l", "start": 1, "end": -1})
	if !reflect.DeepEqual(out["value"], []any{"z"}) || out["len"] != 1 {
		t.Fatalf("slice: %v", out)
	}
	if out := call("len", map[string]any{"key": "l"}); out["len"] != 3 {
		t.Fatalf("len: %v", out)
	}
	if out := call("pop", map[string]any{"key": "l", "from": "front"}); out["value"] != "y" || out["len"] != 2 {
		t.Fatalf("pop front: %v", out)
	}
	if out := call("pop", map[string]any{"key": "l"}); out["value"] != "w" {
		t.Fatalf("pop back default: %v", out)
	}
	if out := call("pop", map[string]any{"key": "drained"}); out["found"] != false {
		t.Fatalf("pop absent: %v", out)
	}

	// delete + list with prefix.
	call("set", map[string]any{"key": "seen-1", "value": 1})
	call("set", map[string]any{"key": "seen-2", "value": 2})
	call("delete", map[string]any{"key": "seen-1"})
	out = call("list", map[string]any{"prefix": "seen-"})
	if !reflect.DeepEqual(out["keys"], []any{"seen-2"}) {
		t.Fatalf("list: %v", out)
	}

	// ttl: parses, expires.
	call("set", map[string]any{"key": "blip", "value": 1, "ttl": "15ms"})
	time.Sleep(25 * time.Millisecond)
	if out := call("get", map[string]any{"key": "blip"}); out["found"] != false {
	...
	for _, bad := range []struct {
		...
		{"set", map[string]any{"key": "k", "value": 1, "ttl": "bogus"}, "bad ttl"},
		{"setnx", map[string]any{"key": "k", "value": 1, "ttl": "bogus"}, "bad ttl"},
		{"pop", map[string]any{"key": "l", "from": "middle"}, "front or back"},
		{"index", map[string]any{"key": "l", "index": "one"}, "must be an integer"},
		{"slice", map[string]any{"key": "l", "start": "one"}, "must be an integer"},
		{"nosuch", map[string]any{}, `no verb "nosuch"`},
	} {
