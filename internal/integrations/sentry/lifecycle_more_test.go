...
func TestStartSmeeBranch(t *testing.T) {
	sse := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, "data: {\"sentry-hook-resource\":\"issue\",\"body\":{\"action\":\"created\",\"data\":{\"issue\":{\"shortId\":\"PROJ-9\",\"title\":\"boom\",\"culprit\":\"c\",\"level\":\"error\",\"project\":{\"slug\":\"api\"},\"permalink\":\"https://s/9\"}}}}\n\n")
...
func TestProjectSlugShapes(t *testing.T) {
	if got := projectSlug(map[string]any{"project": "flat"}); got != "flat" {
		t.Fatalf("flat: %q", got)
	}
	if got := projectSlug(map[string]any{"project": map[string]any{"slug": "nested"}}); got != "nested" {
		t.Fatalf("nested: %q", got)
	}
	if got := projectSlug(map[string]any{"project": 7}); got != "" {
